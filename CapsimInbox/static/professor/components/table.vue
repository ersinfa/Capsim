  ...
  <div class="good-table">
    ...
    <table ref="table" :class="styleClass">
        ...
        <tr v-if="globalSearch && externalSearchQuery == null">
          <td :colspan="lineNumbers ? columns.length + 1: columns.length">
            <div class="input-group">
              ...
              <input type="text" aria-label="Search Table" class="form-control global-search-input" id="global-search-input" :placeholder="globalSearchPlaceholder" v-model="globalSearchTerm" @keyup.enter="searchTable()" />
            </div>
          </td>
        </tr>
        <tr>
          ...
          <th v-for="(column, index) in columns"
            ...
            v-if="!column.hidden">
            <span v-if="column.tooltip" class="glyphicon glyphicon-info-sign" data-toggle="tooltip" data-placement="top" :title="column.tooltip"></span>
      ...
      <tbody>
        <tr v-for="(row, index) in paginated" :class="onClick ? 'clickable' : ''" @click="click(row, index)">
          ...
          <slot name="table-row" :row="row" :index="index">
            <td v-for="(column, i) in columns" :class="getDataStyle(i, 'td')" v-if="!column.hidden">
              <span v-if="!column.html && !column.link">{{ collectFormatted(row, column) }}</span>
              <span v-if="column.html && !column.link" v-html="collect(row, column.field)"></span>
              <!-- <span v-if="column.link" v-html="row[column.field].text"></span> -->
              <router-link v-if="column.link" :to="row[column.field].link">{{row[column.field].linkText}}</router-link>
              <!-- <router-link v-if="column.link" :to="row[column.field].link" :class="row[column.field].linkClass">{{row[column.field].linkText}}</router-link> -->
            </td> 
          </slot>
        </tr>
       ...
       <tfoot>
        <tr v-for="(row, index) in footer" :class="onClick ? 'clickable' : ''" @click="click(row, index)">
          <th v-if="lineNumbers" class="line-numbers">{{ getCurrentIndex(index) }}</th>
          <slot name="table-row" :row="row" :index="index">
            <td v-for="(column, i) in columns" :class="getDataStyle(i, 'td')" v-if="!column.hidden">
              <span v-if="!column.html && !column.link">{{ collectFormatted(row, column) }}</span>
