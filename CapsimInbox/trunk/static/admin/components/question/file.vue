<template>
  <tr id="drive-file">
    ...
    <template v-else>
      ...
      <td>
        <a v-if="file.FK_fileTypeKey == 2" target="_blank" class="btn mat btn-info" :href="file.fileName">Link</a>
        <a v-else target="_blank" class="btn mat btn-info" :href="`${$store.state.assetsPath}/capsiminbox/${fileTypeFolder}/${file.fileName}`">Link</a>
      </td>
      <td>
